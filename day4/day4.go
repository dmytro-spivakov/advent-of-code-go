...
func cardFromLine(inputLine string) card {
	...
	for _, winningNumberString := range winningNumberStrings {
		...
		winningNumber, err := strconv.ParseInt(winningNumberString, 10, 64)
		if err != nil {
			log.Fatal(err)
		}

		winningNumbersMap[int(winningNumber)] = true
	}

	yourNumbers := []int{}
	for _, yourNumber := range yourNumberStrings {
		yourNumber = strings.TrimSpace(yourNumber)
		if yourNumber == "" {
			continue
		}

		yourNumber, err := strconv.ParseInt(strings.TrimSpace(yourNumber), 10, 64)
		if err != nil {
			log.Fatal(err)
		}
